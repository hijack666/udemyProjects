...
function _createClass(Constructor, protoProps, staticProps) { if (protoProps) _defineProperties(Constructor.prototype, protoProps); if (staticProps) _defineProperties(Constructor, staticProps); return Constructor; }
 ...
 // вместо гетРесурс используем библиотеку axios
...
function cards() {
    ...
    _createClass(MenuCard, [{
      ...
      value: function render() {
        ...
        if (this.classes.length === 0) {
          ...
          }); // передаем класс каждому элементу псевдомассива 
        }
  ...
  axios.get('http://localhost:3000/menu') // .then(data => console.log(data)) // Получаем сразу объект
  .then(function (data) {
...
function forms(formSelector, modalTimerId) {
  ...
  function bindPostData(form) {
    form.addEventListener('submit', function (e) {
      e.preventDefault();
      var statusMsg = document.createElement('img'); // statusMsg.classList.add('status');

      statusMsg.src = message.loading;
      statusMsg.style.cssText = 'display: block; margin: 0 auto;'; // statusMsg.textContent = message.loading;

      form.append(statusMsg);
      form.insertAdjacentElement('afterend', statusMsg); // располагаем спиннер после формы

      var formData = new FormData(form);
      var object = {}; // Новая переменная json

      var json = JSON.stringify(Object.fromEntries(formData.entries())); //Сначала превращам в массив массивов, затем в объект и затем снова в json

      formData.forEach(function (value, key) {
        // Перебираем формДату и превращаем в json
        object[key] = value;
      });
...
function slider(_ref) {
  ...
  var width = window.getComputedStyle(slidesWrapper).width; //получили ширину 
  ...
  dots.forEach(function (dot) {
    dot.addEventListener('click', function (e) {
      ...
      sliderIndex = slideTo; // offset = +width.slice(0, width.length - 2) * (slides.length - 1);
...
function tabs(tabsSelector, tabsContentSelector, tabsParentSelector, activeClass) {
  ...
  hideTabContent(); // showTabContent(0); //дефолтное значение, либо при создании функции прописать аргумент "i = 0", а вызывать без аргумента
  ...
  tabsParent.addEventListener('click', function (event) {
    ...
    if (target && target.classList.contains(tabsSelector.slice(1))) {
      // условие что не кликаем на родителя
...
function timer(id, deadline) {
  ...
  function getTimeRemaining(endtime) {
    var t = Date.parse(endtime) - Date.parse(new Date()),
        // endtime количество мс в конечном времени, отнимаем текущую дату
    days = Math.floor(t / (1000 * 60 * 60 * 24)),
        // количество дней из миллисекунд
    hours = Math.floor(t / (1000 * 60 * 60) % 24),
        // остаток, чтобы не получилось 100500 часов
...
__webpack_require__(/*! es6-promise */ "./node_modules/es6-promise/dist/es6-promise.js").polyfill(); // полифилл для es6 промисов, которые не работают в IE


 // Тоже для IE, запись имеет свою особенность - пакет подключится именно в этот скриптовый файл
// ТАБЫ
...
window.addEventListener('DOMContentLoaded', function () {
  ...
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_7__.default)('.timer', '2020-10-30');
  (0,_modules_modal__WEBPACK_IMPORTED_MODULE_3__.default)('[data-modal]', '.modal', modalTimerId);
  (0,_modules_modal__WEBPACK_IMPORTED_MODULE_3__.openModal)('.modal', modalTimerId);
  (0,_modules_modal__WEBPACK_IMPORTED_MODULE_3__.closeModal)('.modal', modalTimerId);
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.next = 2;
            return fetch(url, {
              // Добавляя await - ждем выполнение этой операции
              method: "POST",
              headers: {
